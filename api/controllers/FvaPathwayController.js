...
module.exports = {
	...
	create: function(req, res, next) {
			...
			FvaPathway.create(fvapathwayObj, function mtnlCreated(err, mtnl) {
				if (err) {
					console.log(err);
					req.session.flash = {
					  err: err
					...
					return res.redirect('/fvapathway/new');
				}
				mtnl.save(function(err, mtnl) {
					if (err) return next(err);
					// add the action attribute to the user object for the flash message.
					mtnl.action = " fva pathway created."
					// Let other subscribed sockets know that the user was created.
					FvaPathway.publishCreate(mtnl);
					// After successfully creating the user
					// redirect to the show action
					// From ep1-6: //res.json(user); 
					res.redirect('/fvapathway/show/' + mtnl.id);
				});
			});
	},
	show: function(req, res, next) {
		if (req.session.authenticated){
			FvaPathway.findOne(req.param('id'), function foundLb(err, ptw) {
				if (err) return next(err);
				if (!ptw) return next();
	...
	index: function(req, res, next) {
		if (req.session.authenticated){
			...
			FvaPathway.find(function foundMtns(err, ptws) {
				if (err) return next(err);
	...
	update: function(req, res, next) {
		if (req.session.authenticated){
			var username = req.session.User.name;
			FvaPathway.findOne(req.param('id'), function foundPtw(err, ptw) {
				if (err) return next(err);
				if (!ptw) return next();
				...
				Lab.find(function(err2, labs) {
					...
					if(isEditable){
						FvaPathway.update(req.param('id'), fvapathwayObj, function ptwUpdated(err) {
							res.redirect('/fvapathway/show/' + ptw.id);
						});
					}
					else return next();
				});
	...
	subscribe: function(req, res) {
		...
		FvaPathway.find(function foundPath(err, ps) {
		  if (err) return next(err);
	 
		  // subscribe this socket to the User model classroom
		  FvaPathway.subscribe(req.socket);
	 
		  // subscribe this socket to the user instance rooms
		  FvaPathway.subscribe(req.socket, ps);
